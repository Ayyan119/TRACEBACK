...
export default function KnowledgeDocumentsPage() {
  ...
  useEffect(() => {
    api.getKnowledge({ type: 'runbook' }).then((data) => {
      setDocuments(data);
      setIsLoading(false);
    });
  }, []);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="border-b border-borderColor pb-4">
        <h1 className="text-xl font-bold text-textPrimary">Technical Runbooks & Specs</h1>
        <p className="text-xs text-textSecondary mt-1">Indexed operational documentation and emergency playbooks.</p>
      </div>
          ...
          {documents.map((doc) => (
            <Card key={doc.id} className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <FileCode className="w-5 h-5 text-accentPrimary" />
                <div>
